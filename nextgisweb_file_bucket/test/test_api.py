...
def test_archive(ngw_webtest_app):
    ...
    def make_archive(files):
        ...
        with zipfile.ZipFile(
            data, mode="a", compression=zipfile.ZIP_DEFLATED, allowZip64=False
        ) as archive:
            for f in files:
    ...
    bucket_data = {
        "resource": {
            "cls": "file_bucket",
            "display_name": "test-bucket-from-archive",
            "parent": {"id": 0},
        },
    ...
    with zipfile.ZipFile(
        BytesIO(resp.body), mode="r", compression=zipfile.ZIP_DEFLATED, allowZip64=False
    ) as archive:
